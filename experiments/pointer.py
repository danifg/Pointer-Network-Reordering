...
from neuronlp2.models import DeepBiAffine, NeuroMST, L2RPtrNet
...
def get_optimizer(parameters, optim, learning_rate, lr_decay, betas, eps, amsgrad, weight_decay, warmup_steps):
    if optim == 'sgd':
        ...
        optimizer = Adam(parameters, lr=learning_rate, betas=betas, weight_decay=weight_decay, eps=eps)
    init_lr = 1e-7
...
def eval(alg, data, network, pred_writer, gold_writer, punct_set, word_alphabet, pos_alphabet, device, beam=1, batch_size=256):
    ...
    for data in iterate_data(data, batch_size):
        ...
        if alg == 'graph':
            ...
            heads_pred, types_pred = network.decode(words, chars, bert, postags, mask=masks, leading_symbolic=conllx_data.NUM_SYMBOLIC_TAGS)
        else:
            masks = data['MASK_ENC'].to(device)
            heads_pred, types_pred = network.decode(words, chars, bert, postags, mask=masks, beam=beam, leading_symbolic=conllx_data.NUM_SYMBOLIC_TAGS)
...
def train(args):
    ...
    bert_path_train = args.bert_path_train                                                                                                                    
    bert_path_dev = args.bert_path_dev                                                                                                                        
    bert_path_test = args.bert_path_test
    ...
    alg = 'transition' if model_type == 'L2RPtr' else 'graph'
    if model_type == 'DeepBiAffine':
        ...
        network = L2RPtrNet(word_dim, num_words, char_dim, num_chars, bert_dim, pos_dim, num_pos, mode, hidden_size,
                              ...
                              p_in=p_in, p_out=p_out, p_rnn=p_rnn, pos=use_pos, bert=use_bert, grandPar=grandPar, sibling=sibling)
    else:
    ...
    if alg == 'graph':
        ...
        data_train = conllx_stacked_data.read_bucketed_data(train_path, bert_path_train, bert_dim, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, prior_order=prior_order)
        data_dev = conllx_stacked_data.read_data(dev_path, bert_path_dev, bert_dim, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, prior_order=prior_order)
        data_test = conllx_stacked_data.read_data(test_path, bert_path_test, bert_dim, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, prior_order=prior_order)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step, data in enumerate(iterate_data(data_train, batch_size, bucketed=True, unk_replace=unk_replace, shuffle=True)):
            ...
            if alg == 'graph':
                ...
                loss_arc, loss_type = network.loss(words, chars, bert, postags, heads, types, mask=masks)
            else:
                ...
                loss_arc, loss_type = network.loss(words, chars, bert, postags, heads, stacked_heads, children, siblings, stacked_types,
...
def parse(args):
    ...
    device = torch.device('cuda', 0) if args.cuda else torch.device('cpu')
    test_path = args.test
    remove_cycles= args.remove_cycles
    ...
    if model_type == 'DeepBiAffine':
        ...
        grandPar = hyps['grandPar']
        sibling = hyps['sibling']
        network = L2RPtrNet(word_dim, num_words, char_dim, num_chars, bert_dim, pos_dim, num_pos,
                              ...
                              pos=use_pos, bert=use_bert, grandPar=grandPar, sibling=sibling, remove_cycles=remove_cycles)
    else:
        raise RuntimeError('Unknown model type: %s' % model_type)
    ...
    logger.info("Reading Data")
    if alg == 'graph':
        data_test = conllx_data.read_data(test_path, bert_path_test, bert_dim, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, symbolic_root=True)
    else:
        data_test = conllx_stacked_data.read_data(test_path, bert_path_test, bert_dim, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, prior_order=prior_order)
...
if __name__ == '__main__':
    args_parser = argparse.ArgumentParser(description='Pointer Network with BERT')
    ...
    args_parser.add_argument('--remove_cycles', action='store_true', help='remove cycles')
    #BERT                                                                                                                                                     
    args_parser.add_argument('--bert', action='store_true', help='use BERT embedding.')                                                                       
    args_parser.add_argument('--bert_dim', type=int, default=768, help='Dimension of BERT embeddings')                                                         
    args_parser.add_argument('--bert_path_train', help='path for BERT embeddings train')                                                                      
    args_parser.add_argument('--bert_path_dev', help='path for BERT embeddings dev')                                                                          
    args_parser.add_argument('--bert_path_test', help='path for BERT embeddings test') 
