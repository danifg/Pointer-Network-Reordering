...
def read_data(source_path, bert_path, bert_dim, word_alphabet, char_alphabet, pos_alphabet, type_alphabet,
    ...
    counter = 0
    reader = CoNLLXReader(source_path, bert_path, bert_dim, word_alphabet, char_alphabet, pos_alphabet, type_alphabet)
    ...
    while inst is not None and (not max_size or counter < max_size):
        ...
        data.append([sent.word_ids, sent.char_id_seqs, inst.bert_embs, inst.pos_ids, inst.heads, inst.type_ids, stacked_heads, children, siblings, stacked_types, skip_connect])
    ...
    bert_inputs = np.empty([data_size, max_length, bert_dim], dtype=np.float32) #BERT 
    #bert_inputs = np.empty([data_size, max_length], dtype=np.float32) #BERT
    ...
    data_tensor = {'WORD': words, 'CHAR': chars, 'BERT': berts, 'POS': pos, 'HEAD': heads, 'TYPE': types, 'MASK_ENC': masks_e,
...
def read_bucketed_data(source_path, bert_path, bert_dim, word_alphabet, char_alphabet, pos_alphabet, type_alphabet,
                       max_size=None, normalize_digits=True, prior_order='inside_out'):
    ...
    while inst is not None and (not max_size or counter < max_size):
        ...
        for bucket_id, bucket_size in enumerate(_buckets):
            if inst_size < bucket_size:
                stacked_heads, children, siblings, stacked_types, skip_connect = _generate_stack_inputs(inst.heads, inst.type_ids, prior_order)
                data[bucket_id].append([sent.word_ids, sent.char_id_seqs, inst.bert_embs, inst.pos_ids, inst.heads, inst.type_ids, stacked_heads, children, siblings, stacked_types, skip_connect])
                max_len = max([len(char_seq) for char_seq in sent.char_seqs])
