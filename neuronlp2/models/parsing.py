...
class L2RPtrNet(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, bert_dim, pos_dim, num_pos, rnn_mode, hidden_size,
                 ...
                 pos=True, bert=True, prior_order='inside_out', grandPar=False, sibling=False, activation='elu', remove_cycles=False):
    ...
    def _get_encoder_output(self, input_word, input_char, input_bert, input_pos, mask=None):
        # [batch, length, word_dim]
        word = self.word_embed(input_word)
        ...
        if self.bert:                                                                                                                                         
            enc = torch.cat([enc, input_bert], dim=2)
    ...
    def loss(self, input_word, input_char, input_bert, input_pos, heads, stacked_heads, children, siblings, stacked_types, mask_e=None, mask_d=None):
        # output from encoder [batch, length_encoder, hidden_size]
        output_enc, hn = self._get_encoder_output(input_word, input_char, input_bert, input_pos, mask=mask_e)
    ...
    def decode(self, input_word, input_char, input_bert, input_pos, mask=None, beam=1, leading_symbolic=0):
        ...
        output_enc, hn = self._get_encoder_output(input_word, input_char, input_bert, input_pos, mask=mask)
        enc_dim = output_enc.size(2)
